...
import shards.OrderByColumn;
import shards.WrongShardsQueryException;
 ...
 * required. An application can create a result set using the following code:
 * 
 * <pre>
...
public class SimpleResultSet implements ResultSet, ResultSetMetaData {
	 ...
	 * @throws SQLException
	 */
	public synchronized void addColumn(String name, int sqlType, int precision, int scale) throws SQLException {
...
//		columns = null;
	 ...
	 * 
	 * @throws SQLException
	 *             is this method is not supported
	 */
	...
	/**
	 * Wszystkie kolumny muszą być Comparable!!!
	 */
	@SuppressWarnings("unchecked")
	public synchronized void sort(List<OrderByColumn> columnsIndexesList) {
		Assert.notNull(columnsIndexesList);
		if(columnsIndexesList.size() > 0) {
			Collections.sort(this.rows, new RowComparator(columnsIndexesList));
		}
	}
	
/*	private Iterable<Integer> extractIndexes(String... columnsNames) {
		final List columnsList = this.columns;
		return Iterables.transform(ImmutableList.of(columnsNames), new Function<String, Integer>() {
			public Integer apply(String columnName) {
				for (int t = 0; t < columnsList.size(); t++) {
					Column column = (Column) columnsList.get(t);
					if (column.name.equals(columnName)) {
						return t;
					}
				}
				throw new RuntimeException("Column " + columnName + " is not available in the resultSet");
			}
		});
	}*/
	...
	// #ifdef JDK16
	/*
	 * public RowId getRowId(int columnIndex) throws SQLException { throw
	...
	// #ifdef JDK16
	/*
	 * public RowId getRowId(String columnName) throws SQLException { throw
	 * getUnsupportedException(); }
	 */
	// #endif
	 ...
	 * SQLException { throw getUnsupportedException(); }
	 */
	// #endif
	...
	// #ifdef JDK16
	/*
	 * public NClob getNClob(int columnIndex) throws SQLException { throw
	 * getUnsupportedException(); }
	 */
	// #endif
	...
	// #ifdef JDK16
	/*
	 * public NClob getNClob(String columnName) throws SQLException { throw
	 * getUnsupportedException(); }
	 */
	// #endif
	...
	// #ifdef JDK16
	/*
	 * public SQLXML getSQLXML(int columnIndex) throws SQLException { throw
	 * getUnsupportedException(); }
	 */
	// #endif
	...
	// #ifdef JDK16
	/*
	 * public SQLXML getSQLXML(String columnName) throws SQLException { throw
	 * getUnsupportedException(); }
	 */
	// #endif
	...
	// #ifdef JDK16
	/*
	 * public <T> T unwrap(Class<T> iface) throws SQLException { throw
	 * getUnsupportedException(); }
	 */
	// #endif
	...
	// #ifdef JDK16
	/*
	 * public boolean isWrapperFor(Class<?> iface) throws SQLException { throw
	 * getUnsupportedException(); }
	 */
	// #endif
	...
	public synchronized void removeAddedColumns(int columnCount) {
		while (columns.size() > 0 && columnCount > 0) {
			int lastIdx = columns.size() - 1;
			columns.remove(lastIdx);
			columnCount--;
		}
	}

	private final class RowComparator implements Comparator<Object[]> {
		private final List<OrderByColumn> columnsIndexesList;

		private RowComparator(List<OrderByColumn> columnsIndexesList) {
			this.columnsIndexesList = columnsIndexesList;
		}
		...
		@SuppressWarnings("unchecked")
		private int compare(Object[] o1, Object[] o2, int num) {
			OrderByColumn orderBy = columnsIndexesList.get(num);
			final Integer idx = orderBy.getColumnIndex() - 1;
			try {
				final Comparable c1 = (Comparable) o1[idx];
				final Comparable c2 = (Comparable) o2[idx];
				...
				if (compare != 0) {
					...
					// kolumny takie same wiec porownaj kolejna kolumne
					num++;
					boolean hasMoreColumns = columnsIndexesList.size() > num;
					if (hasMoreColumns) {
						return compare(o1, o2, num);
					} else {
						return 0;
					}
				}
			} catch (ClassCastException e) {
				throw new WrongShardsQueryException("Column with index " + idx + " cannot be sorted because it is not Comparable");
			}
		}
	}
	...
	public void limit(Long limitLong, long offsetLong) {
		...
		if(limitLong != null) {		
			...
			if(limit <= 0) {
				...
				rows = rows.subList(0, Math.min(rows.size(), limit));
			}
		}
	}
